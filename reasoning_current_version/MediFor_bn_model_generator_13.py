...
def input_validation(query_dict, dataframe, log):
    for algorithm in query_dict['algorithms']:
        if(not any(algorithm in column for column in dataframe.columns)):
            log.info(algorithm + ' not in training dataframe')
            print(dataframe.columns)
            print(algorithm + ' not in training dataframe')
            return False
    if(not query_dict['algorithms']):
        log.info('Input missing algorithms list')
        print('Input missing algorithms list')
        return False
    if(not query_dict['image']):
        log.info('Input missing query image')
        print('Input missing query image')
        return False
    query_dict_keys = list(query_dict.keys())
    print(query_dict_keys)
    query_dict_keys.remove('algorithms')
    query_dict_keys.remove('image')
    print(query_dict_keys)
    for algorithm in query_dict_keys:
        if(not any(algorithm in column for column in dataframe.columns)):
            log.info(algorithm + ' is present in input data but not in training data.')
            print(algorithm + ' is present in input data but not in training data.')
            return False
    return True
...
def create_evidence_file(node_dict):
    ...
    evidence_file = 'MediFor_libra_evidence.ev'
...
def run_inference(query_dict, log):
    ...
    if(not input_validation(query_dict, df, log)):
        sys.exit()
    ###################################
